...
from .models import Course, UserProgress, Certificate
...
def company_registration(request):
    if request.method == 'POST':
        ...
        company = Reg_company.objects.create_user(company_name=company_name, company_email=company_email,
                                                  company_password=company_password, company_phone=company_phone,company_address=company_address, company_country=company_country)
...
def Add_quiz(request):
    if request.method == "POST":
        ...
        quize = Quiz(coursename=coursename,question=question,answer=answer)
        quize.save()
...
def quiz_check(request, id):
    ...
    if request.method == 'POST':
        ...
        message = ''
    context = {'quiz': quiz, 'message': message}
    return render(request, 'quiz.html', context)